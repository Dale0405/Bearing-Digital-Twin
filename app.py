...
if page == "Test Setup":
    ...
    with left:
        ...
        bearing_id = r1c2.number_input("", value=40.0, key="bearing_id", label_visibility="collapsed")

        r2c1, r2c2 = st.columns([1,1])
        r2c1.markdown('<div class="param-row">OD (mm)</div>', unsafe_allow_html=True)
        bearing_od = r2c2.number_input("", value=90.0, key="bearing_od", label_visibility="collapsed")

        r3c1, r3c2 = st.columns([1,1])
        r3c1.markdown('<div class="param-row">Width (mm)</div>', unsafe_allow_html=True)
        bearing_width = r3c2.number_input("", value=23.0, key="bearing_width", label_visibility="collapsed")

        r4c1, r4c2 = st.columns([1,1])
        r4c1.markdown('<div class="param-row">Ball Diameter (mm)</div>', unsafe_allow_html=True)
        ball_diameter = r4c2.number_input("", value=15.88, key="ball_diameter", label_visibility="collapsed")

        r5c1, r5c2 = st.columns([1,1])
        r5c1.markdown('<div class="param-row">Number of Balls</div>', unsafe_allow_html=True)
        number_of_balls = r5c2.number_input("", value=8, key="number_of_balls", label_visibility="collapsed")

        r6c1, r6c2 = st.columns([1,1])
        r6c1.markdown('<div class="param-row">Dynamic Load Cr (N)</div>', unsafe_allow_html=True)
        dynamic_rating = r6c2.number_input("", value=31500.0, key="dynamic_rating", label_visibility="collapsed")

        r7c1, r7c2 = st.columns([1,1])
        r7c1.markdown('<div class="param-row">Static Load Co (N)</div>', unsafe_allow_html=True)
        static_rating = r7c2.number_input("", value=24000.0, key="static_rating", label_visibility="collapsed")    # ----------------------------
